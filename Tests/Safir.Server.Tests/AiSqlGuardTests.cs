...
namespace Safir.Server.Tests
{
    ...
    public class AiSqlGuardTests
    {
        ...
        [InlineData("SELECT TOP 5 p.NAME, h.* FROM PERSONEL p JOIN PHOKM h ON h.CODE = p.CODE")]
        [InlineData("SELECT * FROM dbo.LIST_SALARY_END(14050601, 14050631)")]
        ...
        // حقوق قدیمی WPF: در آزمون عمیق مدل از PERSONEL/PHOKM حقوق پنج نفر را با نام گفت
        [InlineData("PERSONEL", true)]
        [InlineData("dbo.PHOKM", true)]
        [InlineData("WORKING", true)]
        [InlineData("LIST_SALARY_END", true)]
        [InlineData("listsalary_end_main", true)]
        [InlineData("Q_FISH_MANDA", true)]
        [InlineData("SELECT_HOKM", true)]
        // viewهای فروش و انبار که فقط برای نام کاربر به SALA_DTL وصل‌اند نباید بسته شوند
        [InlineData("QSL_INVOICE_FROOSH", false)]
        [InlineData("RASID_ANBAR", false)]
