namespace Safir.Shared.Models.CostClose
{
    ...
    public class CostExceptionDto
    {
        ...
        /// <summary>
        /// جهتِ مغایرتِ افتتاحیه، همان‌طور که S05 هنگام ساختن CHK-02 تشخیص
        /// داده. NULL یعنی مغایرت از گردشِ خودِ ماه است، نه از افتتاحیه.
        ///     ۱ = کاردکس موجودی اول دوره دارد ولی سند افتتاحیه صادر نشده
        ///     ۲ = سند افتتاحیه هست ولی کاردکس موجودی اول دوره ندارد
        /// هیچ‌کدام خودکار قابل اصلاح نیستند؛ تصمیم با انبار/حسابداری است.
        /// </summary>
        public byte?   OpeningKind   { get; set; }

        /// <summary>
        /// فقط برای CHK-02: این مغایرت از افتتاحیه می‌آید، نه از گردش ماه.
        /// این‌ها بخش عمده‌ی مغایرت‌های CHK-02 هستند و معمولاً همه‌شان با یک
        /// سند اصلاحیِ دسته‌جمعی رفع می‌شوند، پس جدا نشان داده می‌شوند.
        ///
        /// از <see cref="OpeningKind"/> می‌آید. برای سطرهای قدیمی که پیش از
        /// افزوده‌شدنِ آن ستون ثبت شده‌اند، سرور به معیارِ قبلی برمی‌گردد
        /// («تنها سندِ این حساب، افتتاحیه است») — نگاه کنید
        /// CostCloseController.GetExceptions.
        /// </summary>
