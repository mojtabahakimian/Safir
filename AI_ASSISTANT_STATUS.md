...
## لاگ تشخیصی (۱۴۰۵/۰۷/۰۴)
- `AI_ChatLog` بدون تغییر جدول، Kindهای تازه: 3 درخواست به مدل (مدل جواب‌دهنده، HTTP، خطای خام، توکن، زمان)، 4 خروجی ابزار (همان که مدل دید، ≤۸۰۰۰)، 5 خطای داخلی (متن کامل). خطای ابزارِ `Fail` هم در `DenyReason` ثبت می‌شود.
- `GET /api/ai/admin/diagnostics?conversationId=…` یک JSON کامل (بدون کلید API)؛ دکمه‌ی «دریافت فایل تشخیص» در `/ai/chat`.
- اگر کاربر مشکلی گزارش کرد: همین فایل را بگیرید و بفرستید.

## حساب‌های خودگردان (۱۴۰۵/۰۷/۰۳، اصلاح صاحب پروژه)
- «کدام حساب کل بانک/بدهکاران است» در نرم‌افزار از فرم «تعریف حساب‌های خودگردان» (WPF: `AUTOMATIC`) تعیین می‌شود و در `SAZMAN` ذخیره می‌شود: `BANKHA`، `BEDEHKAR` (حساب کل)، `ADA`/`APA`… (رشته، مثل `113-1-1`).
- ابزارهای `bank_balances`، `top_debtors`، `credit_limit_breaches` دیگر ۱۱۲/۱۱۵ ثابت ندارند (`SelfAccounts.KolAsync`). اگر خالی باشد، ابزار عدد نمی‌دهد.
- آزموده: ۱۰/۱۰ سؤال بدهکار/بانک/سقف اعتبار.

## بازبینی Codex (۱۴۰۵/۰۷/۰۳) — رفع‌شده، commit نشده
- پوشاندن نام، برچسب‌ها را هم می‌پوشاند (StatusName/KindName/UnitName/ObjectName/ColumnName) ← مدل وضعیت بستن ماه و نام ستون‌ها را نمی‌دید. رفع: `AiNameMasker.IsNameKey`.
- ستون تکراری `NAME_2` در run_sql پوشانده نمی‌شد ← رفع (پسوند `_n` نادیده).
- `asOf` نامعتبر بی‌صدا «امروز» می‌شد ← حالا خطا (`FinArgs.AsOf`).
- با `AiReadOnly`، run_sql همیشه یک دیتابیس ثابت را می‌خواند ولی بقیه دیتابیسِ انتخاب‌شده (`X-DB-Connection`) ← سرور/دیتابیس از انتخاب کاربر، فقط اعتبارنامه از AiReadOnly.
- کلید کش نام‌ها دیتابیس را نداشت ← اضافه شد.
- grader علامت منفی و واحد «میلیون» را نمی‌سنجید ← رفع؛ همه‌ی نتایج قبلی دوباره نمره داده شد: هیچ جواب درستی غلط نشد.
- **عمداً انجام نشده (امنیت، بعداً):** بعد از انقضای کش ۲ ساعته، تاریخچه‌ی گفتگوی قدیمی با نام واقعی به مدل می‌رود.

## مدل‌های رایگان غیر Gemini در 9router (۱۴۰۵/۰۷/۰۳، ۴ سؤال)
- فقط `openrouter/nvidia/nemotron-3-ultra-550b-a55b:free` کار کرد: ۳/۴ درست، ۰ غلط، میانگین ۵۱ث (یکی timeout).
- دور دوم (Cline): `cl/nvidia/nemotron-3-ultra-550b-a55b:free` ۴/۴ (۱۴ث) و در آزمون کامل ۴۱/۴۷ — ۴ شکست از باگ عدد رشته‌ای (`AiToolCall.Int`، رفع شد). `cl/moonshotai/kimi-k3`، `cl/deepseek/deepseek-v4.1-flash`، `cl/anthropic/claude-sonnet-4.6` هر سه ۴/۴ ولی **پولی‌اند**: آزمون کامل Kimi بعد از ۱۲ سؤالِ درست، اعتبار Cline را تمام کرد (۴۰۲).
- `ag/claude-sonnet-4-6` (Antigravity): 9router پارامترهای ابزار را حذف می‌کند (فقط `reason` می‌رسد) ← برای سفیر غیرقابل استفاده.
- بقیه اصلاً جواب ندادند: mistral (tier)، xai (بدون اعتبار)، ollama (۴۰۲)، cloudflare kimi (پلن رایگان نه)، nvidia glm-5.2 (منقضی)، groq llama (۴۰۴) و gpt-oss (سقف ۸۰۰۰ توکن/دقیقه)، sambanova DeepSeek (شلوغ)، zenvo (کانال ندارد)، llm7/cohere (اعتبارنامه ندارد)، longcat (۴۰۴).

## اتصال از ایران با v2ray (بدون Tunnel) — آزموده ۱۴۰۵/۰۷/۰۳
- گوگل (Gemini و Antigravity) به درخواستِ مستقیم از آی‌پی ایران **۴۰۳** می‌دهد.
- 9router برنامه‌ی Node است و «Set system proxy» ویندوز را نمی‌خواند؛ برای همین قبلاً فقط با TUN کار می‌کرد.
- راه درست: داشبورد 9router ← Settings ← Network ← **Outbound Proxy** روشن، `http://127.0.0.1:10809`، No Proxy: `localhost,127.0.0.1`.
- فیلد «پروکسی» در `/ai/settings` سفیر **خالی** بماند؛ سفیر فقط به 9router روی localhost وصل است.
- نتیجه: بانک ۱۷ث، بدهکار ۱۹ث، فروش ۱۴ث، سود فروردین ۱۵ث، «سود این ماه» ۱۱ث و «نامشخص» (درست). عددها همان مقادیر قبلی.
- `/v1/models` در 9router پشت پروکسی حدود ۲۵ ثانیه طول می‌کشد؛ «آزمایش اتصال» سقف ۲۰ ثانیه‌ی ثابت داشت و به‌غلط خطا می‌داد ← حالا از تایم‌اوتِ تنظیمات استفاده می‌کند (کل آزمایش حدود ۸۰ ثانیه).
