                ...
                var result = await _db.ExecuteInTransactionAsync(async (conn, tran) =>
                {
                    ...
                    const string attSql = @"
                        SELECT 
                            ...
                            ISNULL(a.OT_NORMAL_H, 0) AS OT_NORMAL_H, ISNULL(a.OT_HOLIDAY_H, 0) AS OT_HOLIDAY_H, ISNULL(a.OT_ADMIN_H, 0) AS OT_ADMIN_H, ISNULL(a.SHORTAGE_H, 0) AS SHORTAGE_H,
                    ...
                    const string mergeSql = @"
                        ...
                        USING (
                            SELECT * FROM OPENJSON(@JsonData) WITH (
                                ...
                                OT_NORMAL_H DECIMAL(6,2), OT_HOLIDAY_H DECIMAL(6,2), OT_ADMIN_H DECIMAL(6,2), SHORTAGE_H DECIMAL(6,2), LEAVE_DAYS DECIMAL(5,2), ABSENT_DAYS DECIMAL(5,2), MISSION_DAYS DECIMAL(5,2),
                        ...
                        WHEN MATCHED THEN
                            UPDATE SET WORK_DAYS=s.WORK_DAYS, DAYS_TOLID=s.DAYS_TOLID, DAYS_EDARI=s.DAYS_EDARI, DAYS_KHADAMAT=s.DAYS_KHADAMAT, DAYS_FOROSH=s.DAYS_FOROSH,
                                       OT_NORMAL_H=s.OT_NORMAL_H, OT_HOLIDAY_H=s.OT_HOLIDAY_H, OT_ADMIN_H=s.OT_ADMIN_H, SHORTAGE_H=s.SHORTAGE_H, LEAVE_DAYS=s.LEAVE_DAYS, ABSENT_DAYS=s.ABSENT_DAYS, MISSION_DAYS=s.MISSION_DAYS,
                        ...
                        WHEN NOT MATCHED THEN
                            INSERT (PER_ID, EMP_ID, SOURCE, WORK_DAYS, DAYS_TOLID, DAYS_EDARI, DAYS_KHADAMAT, DAYS_FOROSH, OT_NORMAL_H, OT_HOLIDAY_H, OT_ADMIN_H, SHORTAGE_H, LEAVE_DAYS, ABSENT_DAYS, MISSION_DAYS, DAYS, DAYSB, FRID_COUNT, TDAYS, PERF_AMOUNT, TRANSP_AMOUNT, KASR_OTHER)
                            VALUES (@PER_ID, s.EMP_ID, 1, s.WORK_DAYS, s.DAYS_TOLID, s.DAYS_EDARI, s.DAYS_KHADAMAT, s.DAYS_FOROSH, s.OT_NORMAL_H, s.OT_HOLIDAY_H, s.OT_ADMIN_H, s.SHORTAGE_H, s.LEAVE_DAYS, s.ABSENT_DAYS, s.MISSION_DAYS, s.DAYS, s.DAYSB, s.FRID_COUNT, s.TDAYS, s.PERF_AMOUNT, s.TRANSP_AMOUNT, s.KASR_OTHER);";
