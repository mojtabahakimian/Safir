    ...
    public class CostCloseController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<VarianceRowDto>>> GetVariances(int runId)
        {
            const string sql = @"
                ...
                LEFT    JOIN dbo.STUF_DEF s  ON CAST(s.CODE  AS BIGINT) = v.Code
                LEFT    JOIN dbo.STUF_DEF st ON CAST(st.CODE AS BIGINT) = d.TargetCode
                WHERE   v.RunId = @runId
            ...
            int runId, [FromQuery] short? level = null)
        {
            const string sql = @"
                ...
                LEFT   JOIN dbo.STUF_DEF s ON CAST(s.CODE AS BIGINT) = ic.Code
        ...
        public async Task<ActionResult<IEnumerable<ItemMarginDto>>> GetMargins(int runId)
        {
            const string sql = @"
                ...
                LEFT    JOIN dbo.STUF_DEF sb ON CAST(sb.CODE AS BIGINT) = t.BalancingCode
                WHERE   m.RunId = @runId
                ORDER BY m.Profit";
            ...
            int runId, [FromQuery] bool whatIf = true)
        {
            ...
            var res = await _db.DoGetStoreProcedureSQLAsync<dynamic>(
                "dbo.CC_sp_S12b_ApplyMarginTargets",
                new { RunId = runId, Month = run.PeriodMonth,
                      DT1 = run.DateFrom, DT2 = run.DateTo, WhatIf = whatIf },
                commandTimeout: 900);
        ...
        public async Task<IActionResult> Rollback(int runId, [FromBody] RollbackRequest req)
        {
            ...
            try
            {
                var res = await _db.DoGetStoreProcedureSQLAsync<dynamic>(
                    "dbo.CC_sp_Rollback",
                    new { RunId = runId, StepCode = req.StepCode,
                          UserName = CurrentUser, WhatIf = req.WhatIf },
                    commandTimeout: 1800);
        ...
        public async Task<ActionResult<IEnumerable<CostUnitDto>>> GetUnits()
        {
            const string sql = @"
                ...
                LEFT   JOIN dbo.ANBAR n ON n.CODE = a.Anbar
