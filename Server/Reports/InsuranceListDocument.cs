...
public sealed class InsuranceListDocument : IDocument
{
    ...
    public void Compose(IDocumentContainer container) => container.Page(page =>
    {
        ...
        {
            c.Item().AlignCenter().Text("صورت دستمزد، حقوق و مزایای ماهانه").FontSize(14).Bold();
            c.Item().Text($"کارگاه: {_data.WorkshopName} | کارفرما: {_data.EmployerName} | شماره کارگاه: {_data.WorkshopCode} | سال: {_data.PeriodYear} | ماه: {_data.PeriodMonthName}");
        });
        page.Content().PaddingTop(6).Table(table =>
        {
            table.ColumnsDefinition(c =>
            {
                c.ConstantColumn(22); c.RelativeColumn(2.3f); c.RelativeColumn(); c.RelativeColumn();
            ...
            foreach (var e in _data.Rows)
            {
                ...
                Cell(Money(e.BaseDailyWage)); Cell(Money(e.SeniorityDailyBase)); Cell(Money(e.TotalDailyWage));
                Cell(Money(e.MonthlyWage)); Cell(Money(e.OtherSubjectBenefits)); Cell(Money(e.TotalSubjectToInsurance));
                Cell(Money(e.TotalGrossPay)); Cell(Money(e.WorkerPremium)); Cell(Money(e.TaxAmount)); Cell(Money(e.NetPayable));
                Cell(e.WorkDays.ToString("0.##", FaCulture));
            }
