...
END CATCH;";
            ...
            // ⚠️ اینجا عمداً پاک‌سازیِ خودکارِ سطرهای حسابداریِ جامانده انجام
            // *نمی‌شود* (برخلاف SaleReturnRebuildService). دلیلش یک موردِ
            // واقعی است که خلافِ فرضِ آن پاک‌سازی درآمد:
            //
            //   فاکتور ۱۰۵۶ تاریخش ۱۴۰۵/۰۴/۱۳ بود و سندش ۵۷۲۵ با تاریخ
            //   ۱۴۰۵/۰۲/۰۳ — ولی فاکتورهای ۱۰۵۰ تا ۱۰۶۲ همگی ۱۴۰۵/۰۲/۰۳اند
            //   و فاکتورهای واقعیِ تیر از شماره ۴۰۵۶ شروع می‌شوند. یعنی
            //   *تاریخِ فاکتور* غلط بود، نه سند.
            //
            // پاک‌سازیِ خودکار در چنین حالتی سطرِ درست را حذف می‌کند تا در
            // ماهِ غلط دوباره ثبت شود، و بدتر: بعدش تاریخ‌ها با هم می‌خوانند
            // پس CHK-19 هم ساکت می‌شود و خطا نامرئی می‌گردد.
            //
            // کدام تاریخ درست است یک واقعیتِ کسب‌وکاری است، نه چیزی که از
            // داده در بیاید. پس تصمیم با کاربر است: CHK-19 مورد را نشان
            // می‌دهد و دکمه‌ی «اصلاح تاریخ» هر سه تاریخ (حواله، فاکتور، سند)
            // را یکی می‌کند — همان قاعده‌ای که صاحب پروژه گذاشت.
            // DriftedAccountingCleanup برای همان مسیر نگه داشته شده.
