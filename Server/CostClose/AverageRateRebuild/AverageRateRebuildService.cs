        ...
        public async Task<AverageRateRebuildResult> RebuildAsync(long sinceDate = 10101, CancellationToken ct = default)
        {
            ...
            async Task ProcessRowAsync(string code, KardexRow t, AnbarState st, List<string> pending)
            {
                ...
                switch (t.TAG)
                {
                    ...
                    case 4: // برگشت فروش
                    {
                        // ⚠️ عیناً کد اصلی: line?.AVRAGE اینجا مقدار همین
                        // ردیفِ TAG=4 است (نه سند فروش اصلی)، که تا این
                        // لحظه هیچ TAG دیگری آن را ننوشته — یعنی معمولاً صفر.
                        // (استثنا: شاخه‌ی «برگشت نوع ۱» در BuildKardexAsync
                        // عمداً همان ID فروش اصلی را می‌دهد؛ آن‌جا این مقدار
                        // واقعاً MIANِ لحظه‌ی فروش است.)
