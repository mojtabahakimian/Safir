...
`MBKM/MIAN/MOGUDI` مستقل خودش را نگه می‌دارد.
...
> ⚠️ **ولی جریان زمانی مشترک به‌تنهایی کافی نیست.** دو ردیفِ یک حواله
> `DATE_N` یکسان دارند، و `tartib` کد ۶ برابر **۱۰** است در برابر **۱۴**
> برای کد ۵ — یعنی مقصد همیشه قبل از مبدأ پردازش می‌شود، `Touched` هرگز
> به‌موقع ست نمی‌شود، و همان مقدار کهنه‌ای خوانده می‌شود که قرار بود رفع
> شود.
>
> راه‌حل: **فقط در حالت ادغام‌شده**، `tartib` شاخهٔ انتقالیِ ورود از کدِ
> *مبدأ* ساخته شود به‌علاوهٔ نیم واحد:
>
> ```sql
> CAST(ISNULL(tsrc.tartib, 0) AS FLOAT) + 0.5   -- یعنی ۱۴٫۵
> LEFT JOIN dbo.TAGCOD tsrc ON tsrc.CODE = h.TAG
> ```
>
> `LEFT` و نه `INNER` — این JOIN فقط برای گرفتن `tartib` مبدأ است و نباید
> ردیفی را حذف کند.
>
> در حالت انبارِ مشخص همان `tartib` طبیعیِ کد ۶ درست است: دو ردیف هرگز در
> یک نتیجه کنار هم نمی‌آیند و ترتیب را خودِ ترتیبِ انبارها تأمین می‌کند.
>
> روی این پایگاه ۱۶ کالا چرخهٔ واقعی دارند، از جمله ۳۳۶۵ و ۳۷۴ و ۳۶۸.
...
| اختلاف فقط روی کالاهای چرخه‌دار | `tartib` انتقالیِ ورود در حالت ادغام‌شده — بخش ۷ |
