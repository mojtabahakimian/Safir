...
```csharp
if (Math.Abs(MBKM)   < 1e-6) { /* هیچ — MIAN دست‌نخورده می‌ماند */ }
else if (Math.Abs(MOGUDI) < 1e-6) { MBKM = 0; /* MIAN دست‌نخورده */ }
else                              { MIAN = MBKM / MOGUDI; }
```

⚠️ کد اصلی اینجا `== 0` دارد. **آن غلط است** و همان چیزی است که نرخ منفیِ
کد ۳۳۶۵ را ساخت — بخش ۱۲ کامل توضیحش می‌دهد. روی دادهٔ سالم این تغییر
هیچ عددی را عوض نمی‌کند.
...
> ⚠️ **جریان زمانی مشترک به‌تنهایی «مبدأ قبل از مقصد» را تضمین نمی‌کند.**
> دو ردیفِ یک حواله `DATE_N` یکسان دارند و `tartib` کد ۶ برابر **۱۰** است
> در برابر **۱۴** برای کد ۵ — پس مقصد اول پردازش می‌شود، `Touched` ست
> نمی‌شود، و `MABL_K` ذخیره‌شده خوانده می‌شود نه مقدارِ همین دور.
>
> **ولی این را «درست» نکنید.** وسوسهٔ طبیعی این است که `tartib` ورود را
> از کدِ مبدأ + ۰٫۵ بسازید تا بلافاصله بعد از خروج بنشیند. این کار ترتیبِ
> رویدادها را عوض می‌کند، نه فقط تازگیِ یک عدد را — و روی همین دیتابیس
> فاجعه می‌سازد. بخش ۱۲ را ببینید.
>
> کهنه‌بودنِ `MABL_K` در Case 6 مسئلهٔ کوچک‌تری است و ارزش این ریسک را
> ندارد. `tartib` طبیعی را نگه دارید.
...
## ۱۲. مقایسهٔ صفر با تلورانس — نه با `==`

این بخش بعد از یک خرابیِ واقعی اضافه شد و **مهم‌ترین اصلاحِ باقی‌مانده در
برنامهٔ شبانه است**.

### نشانه

کد ۳۳۶۵ در بازسازی اتوباز نرخ منفی گرفت. در سفیر همان داده درست بود.

### علت

قاعدهٔ مشترکِ بازمحاسبه (بخش ۶) صفر را **دقیق** مقایسه می‌کند:

```csharp
if (MBKM == 0)        { }
else if (MOGUDI == 0) { MBKM = 0; }
else                  { MIAN = MBKM / MOGUDI; }
```

مقدارها `double` اند و از جمع و تفریقِ صدها تراکنش ساخته می‌شوند. جایی که
از نظر حسابداری باید **دقیقاً صفر** باشند، در عمل یک باقیماندهٔ ریز
می‌ماند. آن مقایسهٔ دقیق باقیمانده را صفر نمی‌بیند، و خط بعد همان را
می‌کند **مخرجِ تقسیم**.

روی کد ۳۳۶۵ / انبار ۸۰۹ / تاریخ ۱۴۰۵/۰۵/۰۶:

```
tag=5  خروج ۳۸٫۵ از موجودیِ ۱۳٫۵   →  MOGUDI = -24.999999999999993
tag=6  ورود ۲۵                      →  MOGUDI = 7.105427357601002e-15
                                        MBKM   = -2.05
                                        MIAN   = -288,311,948,083,200
```

آن عدد از راه `SMABL` به ۳۱ کالای دیگر سرایت کرد و در ادامه به ‎±۱e۲۶‎
رسید. مغایرت‌هایی که ماه‌ها دنبالشان بودیم از همین‌جا شروع می‌شود.

### اصلاح

```csharp
private const double ZeroEpsilon = 1e-6;   // یک میلی‌گرم

if (Math.Abs(MBKM) < ZeroEpsilon)        { }
else if (Math.Abs(MOGUDI) < ZeroEpsilon) { MBKM = 0; }
else                                     { MIAN = MBKM / MOGUDI; }
```

۱e-6 کیلوگرم را هیچ کاردکسی نمی‌شمارد، پس روی دادهٔ سالم هیچ نتیجه‌ای
عوض نمی‌شود. تست روی کد ۳۳۶۵ عیناً همان نرخ‌های قبلی را داد.

### چرا اتوباز خورد و سفیر نه

خودِ باقیمانده به **ترتیبِ رویدادها** بستگی دارد. اتوباز `tartib`
انتقالیِ ورود را در حالت ادغام‌شده به «کدِ مبدأ + ۰٫۵» تغییر داده بود؛ آن
تغییر انبار ۸۰۹ را از مسیرِ «خروجِ ۳۸٫۵ از موجودیِ ۱۳٫۵» می‌بَرد و دقیقاً
همان باقیمانده را می‌سازد. سفیر با `tartib` طبیعی این مسیر را نمی‌رود.

چهار حالت، روی همین کالا:

| ترتیب | نگهبان تلورانس | نتیجه |
|---|:--:|---|
| طبیعی | ✘ | سالم |
| طبیعی | ✔ | سالم، **عدد به عدد یکسان** |
| ‎+۰٫۵‎ | ✘ | ‎−۹٫۳e۲۶‎ |
| ‎+۰٫۵‎ | ✔ | سالم، ولی نرخ‌ها با ستون اول فرق دارد |

### دو کارِ لازم در برنامهٔ شبانه

۱. **تغییر ‎+۰٫۵‎ را پس بگیرید** و `tartib` طبیعیِ کد ۶ را برگردانید.
۲. **نگهبان تلورانس را اضافه کنید.** حتی با ترتیب درست، هر توالیِ دیگری
   که باقیمانده بسازد همین انفجار را می‌دهد. این یکی مین است، نه یک مورد
   خاص.
