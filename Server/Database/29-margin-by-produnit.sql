/* ═══════════════════════════════════════════════════════════════════
   ...
   ── ماهِ تولید مهم نیست، واحدِ تولید مهم است ──
   کالایی که تیر در یزد تولید و مرداد فروخته شده، تولیدِ یزد است. پس
   جست‌وجوی رسید تولید محدود به همین ماه نیست: اگر همین دوره تولید
   داشته باشد همان ملاک است، وگرنه کل سابقه تا پایان دوره نگاه می‌شود.
   ترتیبش برعکس نیست چون کالایی که تولیدش امسال از یک واحد به واحد دیگر
   منتقل شده باید به واحد فعلی برود، نه واحد قدیمی.

   ── کالایی که هرگز تولید نشده ──
   کالای بازرگانی با ProdUnitId = NULL ثبت می‌شود، نه اینکه کنار گذاشته
   شود. دلیلش همان دلیلِ «بدون واحد» در CC_ItemMarginUnit است: جمعِ
   تفکیکی باید مو‌به‌مو با CC_ItemMargin بخواند، وگرنه گزارش بی‌صدا
   بخشی از فروش را گم می‌کند.
...
BEGIN
    ...
    /* ─── رسیدهای تولید تا پایان همین دوره ───
       ⚠ عمداً محدود به همین ماه نیست. سؤال «کدام واحد این کالا را
       می‌سازد» با «کدام واحد این ماه ساخت» یکی نیست: کالایی که تیر در
       یزد تولید و مرداد فروخته شده، باز هم تولیدِ یزد است. وقتی این
       فیلتر روی ماه بود، چنین کالایی زیر «بدون تولید» می‌افتاد —
       روی اجرای ۱۰، ۹ کالا به ارزش ۶٫۴ میلیارد. */
    Hist AS (
        SELECT  TRY_CAST(pl.CODE AS BIGINT) AS Code,
                au.UnitId                   AS UnitId,
                pl.MEGHK                    AS Qty,
                -- لایه ۰ = تولیدِ همین دوره، ۱ = تولیدِ قبل از آن
                CASE WHEN h.DATE_N BETWEEN @DT1 AND @DT2 THEN 0 ELSE 1 END AS Tier
        FROM    dbo.HEAD_LST h
    ...
    /* تولیدِ همین دوره اگر باشد ملاک است، وگرنه کل سابقه تا پایان دوره.
       عکسش — همیشه کل سابقه — کالایی را که تولیدش امسال از یک واحد به
       واحد دیگر منتقل شده به واحد قدیمی می‌چسباند. */
    Tier AS (
        SELECT Code, MIN(Tier) AS Tier FROM Hist GROUP BY Code
    ),
    /* مقدار منفی (اصلاحیه) با هم جمع می‌شود ولی سطرِ خالص صفر یا منفی
       کنار می‌رود: سهم منفی از تولید، سود را وارونه پخش می‌کرد. */
    Prod AS (
        SELECT  h.Code, h.UnitId, SUM(h.Qty) AS ProdQty
        FROM    Hist h
        JOIN    Tier t ON t.Code = h.Code AND t.Tier = h.Tier
        GROUP BY h.Code, h.UnitId
        HAVING  SUM(h.Qty) > 0
    ),
    Share AS (
        SELECT  p.Code,
