...
BEGIN
    ...
    IF @DEED_MODE = 1
    BEGIN
        -- جمعِ منفیِ یک مرکز فقط از تعدیلِ گِردکردنِ پرسنلِ فقط‌بیمه می‌آید (کارکرد صفر ← سطل
        -- خدمات). در ماهی که مرکز خدمات پرسنلِ واقعی ندارد، جمعش منفی می‌شد و «> 0» ردیف را
        -- حذف می‌کرد؛ سند به همان اندازه ناتراز می‌شد. حالا بستانکارِ هزینه، مثل روش ۲.
        INSERT INTO #FinalArticles
        SELECT CAST(@ACC_SALARY_TOLID AS NVARCHAR(100)), CAST(N'هزینه حقوق تولید ' + @ML AS NVARCHAR(500)), CAST(CASE WHEN SUM(EXP_TOLID) > 0 THEN SUM(EXP_TOLID) ELSE 0 END AS BIGINT), CAST(CASE WHEN SUM(EXP_TOLID) < 0 THEN -SUM(EXP_TOLID) ELSE 0 END AS BIGINT), CAST('EXP_TOLID' AS NVARCHAR(50)), CAST(NULL AS INT), CAST(NULL AS NVARCHAR(150)), 1
        FROM #SalarySplit HAVING SUM(EXP_TOLID) <> 0
        UNION ALL
        SELECT CAST(@ACC_SALARY_EDARI AS NVARCHAR(100)), CAST(N'هزینه حقوق اداری ' + @ML AS NVARCHAR(500)), CAST(CASE WHEN SUM(EXP_EDARI) > 0 THEN SUM(EXP_EDARI) ELSE 0 END AS BIGINT), CAST(CASE WHEN SUM(EXP_EDARI) < 0 THEN -SUM(EXP_EDARI) ELSE 0 END AS BIGINT), CAST('EXP_EDARI' AS NVARCHAR(50)), CAST(NULL AS INT), CAST(NULL AS NVARCHAR(150)), 2
        FROM #SalarySplit HAVING SUM(EXP_EDARI) <> 0
        UNION ALL
        SELECT CAST(@ACC_SALARY_FOROSH AS NVARCHAR(100)), CAST(N'هزینه حقوق فروش ' + @ML AS NVARCHAR(500)), CAST(CASE WHEN SUM(EXP_FOROSH) > 0 THEN SUM(EXP_FOROSH) ELSE 0 END AS BIGINT), CAST(CASE WHEN SUM(EXP_FOROSH) < 0 THEN -SUM(EXP_FOROSH) ELSE 0 END AS BIGINT), CAST('EXP_FOROSH' AS NVARCHAR(50)), CAST(NULL AS INT), CAST(NULL AS NVARCHAR(150)), 3
        FROM #SalarySplit HAVING SUM(EXP_FOROSH) <> 0
        UNION ALL
        SELECT CAST(@ACC_SALARY_KHADAMAT AS NVARCHAR(100)), CAST(N'هزینه حقوق خدمات ' + @ML AS NVARCHAR(500)), CAST(CASE WHEN SUM(EXP_KHADAMAT) > 0 THEN SUM(EXP_KHADAMAT) ELSE 0 END AS BIGINT), CAST(CASE WHEN SUM(EXP_KHADAMAT) < 0 THEN -SUM(EXP_KHADAMAT) ELSE 0 END AS BIGINT), CAST('EXP_KHADAMAT' AS NVARCHAR(50)), CAST(NULL AS INT), CAST(NULL AS NVARCHAR(150)), 4
