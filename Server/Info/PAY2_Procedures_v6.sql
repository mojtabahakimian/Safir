...
-- ================================================================
-- ۲.۴ SP_PAY2_GET_ADVANCES — محاسبه مساعده هوشمند (Refactored)
-- ================================================================
...
BEGIN
    ...
    -- خواندن کد کامل حساب مساعده از تنظیمات
    ...
    DECLARE @MIN_POS   BIT           = ISNULL(CAST((SELECT CFG_VALUE FROM PAY2_CONFIG WHERE CFG_KEY='ADV_MIN_POSITIVE') AS BIT), 1);
    ...
    (
        SELECT
            ...
            dbo.FN_PAY2_RESOLVE_ACCOUNT(@FULL_HES, ISNULL(E.ACC_T, N'')) AS ResolvedAcc,
...
-- ================================================================
-- توابع پایه حسابداری (Account Functions)
-- ================================================================

-- ۱. تابع تولید رشته استاندارد حساب
...
-- ۳. تابع بررسی موجودیت حساب در دیتابیس
...
(
    ...
    SELECT
        FullCode,
        CASE
            WHEN LevelCount = 2 THEN CASE WHEN EXISTS (SELECT 1 FROM DETA_HES WHERE N_KOL = HES_K AND NUMBER = HES_M) THEN 1 ELSE 0 END
        ...
        END AS IsValid
    FROM Parsed
    WHERE LevelCount BETWEEN 2 AND 6 AND HES_K IS NOT NULL AND HES_M IS NOT NULL
);
GO

-- ================================================================
-- ۲.۱ SP_PAY2_GEN_DEED_SUMMARY — تولید سند (مدل فعلی / تجمیعی)
-- ================================================================
...
BEGIN
    ...
    DECLARE @ACC_SALARY_TOLID NVARCHAR(50), @ACC_SALARY_EDARI NVARCHAR(50), @ACC_SALARY_FOROSH NVARCHAR(50), @ACC_SALARY_KHADAMAT NVARCHAR(50);
    DECLARE @ACC_SALARY_PAY NVARCHAR(50), @ACC_INS_PAYABLE NVARCHAR(50), @ACC_TAX_PAYABLE NVARCHAR(50), @ACC_INS_EXP NVARCHAR(50);
    DECLARE @ACC_ADV_HES NVARCHAR(50), @ACC_LOAN_HES NVARCHAR(50), @ACC_OTHER_DED NVARCHAR(50);
    ...
    -- Guard: خالص منفی
    DECLARE @NegEmp NVARCHAR(150);
    SELECT TOP 1 @NegEmp = E.LAST_NAME + N' ' + E.FIRST_NAME + N' (' + CAST(RL.NET_PAY AS NVARCHAR) + N')'
    FROM PAY2_RUN_LINE RL INNER JOIN PAY2_EMPLOYEE E ON RL.EMP_ID = E.EMP_ID
    WHERE RL.RUN_ID = @RUN_ID AND RL.NET_PAY < 0;

    IF @NegEmp IS NOT NULL
    BEGIN
        RAISERROR(N'توقف صدور سند: خالص پرداختی برای پرسنل %s منفی است.', 16, 1, @NegEmp);
        RETURN;
    END

    -- Guard: سایر کسورات نیازمند حساب است
    DECLARE @TotalOtherDed BIGINT;
    SELECT @TotalOtherDed = ISNULL(SUM(OTHER_DED), 0) FROM PAY2_RUN_LINE WHERE RUN_ID = @RUN_ID;

    IF @TotalOtherDed > 0 AND (@ACC_OTHER_DED IS NULL OR LTRIM(RTRIM(@ACC_OTHER_DED)) = N'')
    BEGIN
        RAISERROR(N'توقف صدور سند: مجموع سایر کسورات بیشتر از صفر است اما حساب OTHER_DED_ACCOUNT در تنظیمات کارگاه تعریف نشده است.', 16, 1);
        RETURN;
    END

    -- Guard: Resolver NULL Check (Even for SUMMARY we need valid accounts)
    DECLARE @NullResolverEmp NVARCHAR(200);
    SELECT TOP 1 @NullResolverEmp = E.LAST_NAME + N' ' + E.FIRST_NAME + N' (ID: ' + CAST(E.EMP_ID AS NVARCHAR) + N', ACC_T: ' + ISNULL(E.ACC_T, N'خالی') + N')'
    FROM PAY2_RUN_LINE RL INNER JOIN PAY2_EMPLOYEE E ON RL.EMP_ID = E.EMP_ID
    WHERE RL.RUN_ID = @RUN_ID AND (
        (RL.NET_PAY > 0 AND dbo.FN_PAY2_RESOLVE_ACCOUNT(@ACC_SALARY_PAY, ISNULL(E.ACC_T, N'')) IS NULL) OR
        (RL.LOAN_DED > 0 AND dbo.FN_PAY2_RESOLVE_ACCOUNT(@ACC_LOAN_HES, ISNULL(E.ACC_T, N'')) IS NULL) OR
        (RL.ADVANCE_DED > 0 AND dbo.FN_PAY2_RESOLVE_ACCOUNT(@ACC_ADV_HES, ISNULL(E.ACC_T, N'')) IS NULL)
    );

    IF @NullResolverEmp IS NOT NULL
    BEGIN
        RAISERROR(N'توقف صدور سند: ایجاد حساب شخص‌محور برای پرسنل %s امکان‌پذیر نیست. حساب پایه یا کد تفصیل شخص نامعتبر است.', 16, 1, @NullResolverEmp);
        RETURN;
    END

    ;WITH SalarySplitBase AS (
        SELECT RL.EMP_ID, RL.GROSS_PAY, A.WORK_DAYS, A.DAYS_TOLID, A.DAYS_EDARI, A.DAYS_FOROSH, A.DAYS_KHADAMAT,
        ...
        FROM PAY2_RUN_LINE RL INNER JOIN PAY2_ATTENDANCE A ON RL.EMP_ID = A.EMP_ID AND A.PER_ID = @PER_ID
    ...
    SELECT CAST(@ACC_SALARY_TOLID AS NVARCHAR(100)) AS HES_CODE, CAST(N'هزینه حقوق تولید ' + @ML AS NVARCHAR(500)) AS SHARH, CAST(SUM(EXP_TOLID) AS BIGINT) AS BED, CAST(0 AS BIGINT) AS BES, CAST('EXP_TOLID' AS NVARCHAR(50)) AS ACC_KEY, CAST(NULL AS INT) AS EMP_ID
    FROM SalarySplit HAVING SUM(EXP_TOLID) > 0
    UNION ALL
    SELECT CAST(@ACC_SALARY_EDARI AS NVARCHAR(100)), CAST(N'هزینه حقوق اداری ' + @ML AS NVARCHAR(500)), CAST(SUM(EXP_EDARI) AS BIGINT), CAST(0 AS BIGINT), CAST('EXP_EDARI' AS NVARCHAR(50)), CAST(NULL AS INT)
    FROM SalarySplit HAVING SUM(EXP_EDARI) > 0
    UNION ALL
    SELECT CAST(@ACC_SALARY_FOROSH AS NVARCHAR(100)), CAST(N'هزینه حقوق فروش ' + @ML AS NVARCHAR(500)), CAST(SUM(EXP_FOROSH) AS BIGINT), CAST(0 AS BIGINT), CAST('EXP_FOROSH' AS NVARCHAR(50)), CAST(NULL AS INT)
    FROM SalarySplit HAVING SUM(EXP_FOROSH) > 0
    UNION ALL
    SELECT CAST(@ACC_SALARY_KHADAMAT AS NVARCHAR(100)), CAST(N'هزینه حقوق خدمات ' + @ML AS NVARCHAR(500)), CAST(SUM(EXP_KHADAMAT) AS BIGINT), CAST(0 AS BIGINT), CAST('EXP_KHADAMAT' AS NVARCHAR(50)), CAST(NULL AS INT)
    FROM SalarySplit HAVING SUM(EXP_KHADAMAT) > 0
    UNION ALL
    SELECT CAST(@ACC_INS_EXP AS NVARCHAR(100)), CAST(N'هزینه بیمه کارفرما ' + @ML AS NVARCHAR(500)), CAST(SUM(INS_EMPLOYER) AS BIGINT), CAST(0 AS BIGINT), CAST('INS_EXP' AS NVARCHAR(50)), CAST(NULL AS INT)
    FROM PAY2_RUN_LINE WHERE RUN_ID = @RUN_ID HAVING SUM(INS_EMPLOYER) > 0
    UNION ALL
    SELECT CAST(dbo.FN_PAY2_RESOLVE_ACCOUNT(@ACC_SALARY_PAY, ISNULL(E.ACC_T, N'')) AS NVARCHAR(100)), CAST(N'حقوق پرداختنی: ' + @ML + N' | ' + E.LAST_NAME + N' ' + E.FIRST_NAME AS NVARCHAR(500)), CAST(0 AS BIGINT), CAST(RL.NET_PAY AS BIGINT), CAST('SALARY_PAYABLE' AS NVARCHAR(50)), CAST(E.EMP_ID AS INT)
    FROM PAY2_RUN_LINE RL INNER JOIN PAY2_EMPLOYEE E ON RL.EMP_ID = E.EMP_ID
    WHERE RL.RUN_ID = @RUN_ID AND RL.NET_PAY > 0
    UNION ALL
    SELECT CAST(@ACC_INS_PAYABLE AS NVARCHAR(100)), CAST(N'بیمه تأمین اجتماعی ' + @ML AS NVARCHAR(500)), CAST(0 AS BIGINT), CAST(SUM(INS_WORKER + INS_EMPLOYER) AS BIGINT), CAST('INS_PAYABLE' AS NVARCHAR(50)), CAST(NULL AS INT)
    FROM PAY2_RUN_LINE WHERE RUN_ID = @RUN_ID HAVING SUM(INS_WORKER + INS_EMPLOYER) > 0
    UNION ALL
    SELECT CAST(@ACC_TAX_PAYABLE AS NVARCHAR(100)), CAST(N'مالیات حقوق ' + @ML AS NVARCHAR(500)), CAST(0 AS BIGINT), CAST(SUM(TAX_AMOUNT) AS BIGINT), CAST('TAX_PAYABLE' AS NVARCHAR(50)), CAST(NULL AS INT)
    FROM PAY2_RUN_LINE WHERE RUN_ID = @RUN_ID HAVING SUM(TAX_AMOUNT) > 0
    UNION ALL
    SELECT CAST(dbo.FN_PAY2_RESOLVE_ACCOUNT(@ACC_LOAN_HES, ISNULL(E.ACC_T, N'')) AS NVARCHAR(100)), CAST(N'کسر اقساط وام: ' + @ML + N' | ' + E.LAST_NAME + N' ' + E.FIRST_NAME AS NVARCHAR(500)), CAST(0 AS BIGINT), CAST(RL.LOAN_DED AS BIGINT), CAST('LOAN_HES' AS NVARCHAR(50)), CAST(E.EMP_ID AS INT)
    FROM PAY2_RUN_LINE RL INNER JOIN PAY2_EMPLOYEE E ON RL.EMP_ID = E.EMP_ID
    WHERE RL.RUN_ID = @RUN_ID AND RL.LOAN_DED > 0
    UNION ALL
    SELECT CAST(dbo.FN_PAY2_RESOLVE_ACCOUNT(@ACC_ADV_HES, ISNULL(E.ACC_T, N'')) AS NVARCHAR(100)), CAST(N'تصفیه مساعده: ' + @ML + N' | ' + E.LAST_NAME + N' ' + E.FIRST_NAME AS NVARCHAR(500)), CAST(0 AS BIGINT), CAST(RL.ADVANCE_DED AS BIGINT), CAST('ADVANCE_SETTLE' AS NVARCHAR(50)), CAST(E.EMP_ID AS INT)
    FROM PAY2_RUN_LINE RL INNER JOIN PAY2_EMPLOYEE E ON RL.EMP_ID = E.EMP_ID
    WHERE RL.RUN_ID = @RUN_ID AND RL.ADVANCE_DED > 0
    UNION ALL
    SELECT CAST(@ACC_OTHER_DED AS NVARCHAR(100)), CAST(N'سایر کسورات حقوق ' + @ML AS NVARCHAR(500)), CAST(0 AS BIGINT), CAST(SUM(OTHER_DED) AS BIGINT), CAST('OTHER_DED_ACCOUNT' AS NVARCHAR(50)), CAST(NULL AS INT)
    FROM PAY2_RUN_LINE WHERE RUN_ID = @RUN_ID HAVING SUM(OTHER_DED) > 0;
END;
GO

-- ================================================================
-- ۲.۲ SP_PAY2_GEN_DEED_PERSON — تولید سند (مدل تفصیلی شخص‌محور)
-- ================================================================
...
BEGIN
    ...
    WHERE RL.RUN_ID = @RUN_ID AND (
        (RL.NET_PAY > 0 AND dbo.FN_PAY2_RESOLVE_ACCOUNT(@ACC_SALARY_PAY, ISNULL(E.ACC_T, N'')) IS NULL) OR
        (RL.LOAN_DED > 0 AND dbo.FN_PAY2_RESOLVE_ACCOUNT(@ACC_LOAN_HES, ISNULL(E.ACC_T, N'')) IS NULL) OR
        (RL.ADVANCE_DED > 0 AND dbo.FN_PAY2_RESOLVE_ACCOUNT(@ACC_ADV_HES, ISNULL(E.ACC_T, N'')) IS NULL) OR
        (RL.OTHER_DED > 0 AND dbo.FN_PAY2_RESOLVE_ACCOUNT(@ACC_OTHER_DED, ISNULL(E.ACC_T, N'')) IS NULL)
    );
    ...
    WHERE RL.RUN_ID = @RUN_ID AND RL.ADVANCE_DED > 0
    UNION ALL
    SELECT CAST(dbo.FN_PAY2_RESOLVE_ACCOUNT(@ACC_OTHER_DED, ISNULL(E.ACC_T, N'')) AS NVARCHAR(100)), CAST(N'سایر کسورات حقوق: ' + @ML + N' | ' + E.LAST_NAME + N' ' + E.FIRST_NAME AS NVARCHAR(500)), CAST(0 AS BIGINT), CAST(RL.OTHER_DED AS BIGINT), CAST('OTHER_DED_ACCOUNT' AS NVARCHAR(50)), CAST(E.EMP_ID AS INT)
    FROM PAY2_RUN_LINE RL INNER JOIN PAY2_EMPLOYEE E ON RL.EMP_ID = E.EMP_ID
...
-- ================================================================
-- ۲.۳ SP_PAY2_GEN_DEED (Dispatcher) — مدیریت درخواست صدور سند
-- ================================================================
...
BEGIN
    ...
    IF @DEED_MODE = 1
    BEGIN
        EXEC SP_PAY2_GEN_DEED_SUMMARY @RUN_ID = @RUN_ID, @CALC_BY = @CALC_BY;
    END
    ELSE IF @DEED_MODE = 2
    BEGIN
        EXEC SP_PAY2_GEN_DEED_PERSON @RUN_ID = @RUN_ID, @CALC_BY = @CALC_BY;
    END
    ELSE
    BEGIN
        RAISERROR(N'الگوی صدور سند نامشخص است (@DEED_MODE). فقط 1 و 2 مجاز است.', 16, 1);
        RETURN;
    END
END;
GO
...
BEGIN
    ...
    -- خروجی نهایی برای صدور سند — با ماه در شرح
