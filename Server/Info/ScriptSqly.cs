...
END;
GO
CREATE OR ALTER PROCEDURE [dbo].[SP_PAY2_GEN_DEED_LEGACY_BASELINE]
    @RUN_ID  INT,
    @CALC_BY INT = NULL
AS
BEGIN
    ...
    UNION ALL 
    SELECT 
        CASE WHEN NULLIF(LTRIM(RTRIM(ISNULL(E.ACC_T,N''))),N'') IS NULL THEN @ACC_SALARY_PAY ELSE dbo.FN_PAY2_RESOLVE_PERSON_ACCOUNT(@ACC_SALARY_PAY, E.ACC_T, @ACC_SALARY_PAY) END,
    ...
    UNION ALL 
    SELECT CASE WHEN NULLIF(LTRIM(RTRIM(ISNULL(E.ACC_T,N''))),N'') IS NULL THEN @ACC_LOAN_HES ELSE dbo.FN_PAY2_RESOLVE_PERSON_ACCOUNT(@ACC_LOAN_HES, E.ACC_T, @ACC_SALARY_PAY) END, N'کسر اقساط وام: ' + @ML + N' | ' + E.LAST_NAME + N' ' + E.FIRST_NAME, 0, RL.LOAN_DED, 'LOAN_HES', E.EMP_ID
    FROM PAY2_RUN_LINE RL INNER JOIN PAY2_EMPLOYEE E ON RL.EMP_ID = E.EMP_ID
    WHERE RL.RUN_ID = @RUN_ID AND RL.LOAN_DED > 0
    UNION ALL 
    SELECT CASE WHEN NULLIF(LTRIM(RTRIM(ISNULL(E.ACC_T,N''))),N'') IS NULL THEN @ACC_ADV_HES ELSE dbo.FN_PAY2_RESOLVE_PERSON_ACCOUNT(@ACC_ADV_HES, E.ACC_T, @ACC_SALARY_PAY) END, N'تصفیه مساعده: ' + @ML + N' | ' + E.LAST_NAME + N' ' + E.FIRST_NAME, 0, RL.ADVANCE_DED, 'ADVANCE_SETTLE', E.EMP_ID
    FROM PAY2_RUN_LINE RL INNER JOIN PAY2_EMPLOYEE E ON RL.EMP_ID = E.EMP_ID
