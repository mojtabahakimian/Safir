...
BEGIN
    ...
    BEGIN
        ...
        UNION ALL
        SELECT CAST(@ACC_SALARY_PAY AS NVARCHAR(100)), CAST(N'بدهی بیمه و مالیات پرسنل ' + @ML AS NVARCHAR(500)), CAST(SUM(CASE WHEN NET_PAY < 0 THEN -NET_PAY ELSE 0 END) AS BIGINT), CAST(0 AS BIGINT), CAST('SALARY_PAYABLE' AS NVARCHAR(50)), CAST(NULL AS INT), CAST(NULL AS NVARCHAR(150)), 6
        FROM #SalarySplit HAVING SUM(CASE WHEN NET_PAY < 0 THEN -NET_PAY ELSE 0 END) > 0
        UNION ALL
    ...
    BEGIN
        ...
        UNION ALL
        SELECT CAST(CONCAT_WS('-', @ACC_SALARY_PAY, SUFFIX) AS NVARCHAR(100)), CAST(N'بدهی بیمه و مالیات: ' + @ML + N' | ' + FULL_NAME AS NVARCHAR(500)), CAST(-NET_PAY AS BIGINT), CAST(0 AS BIGINT), CAST('SALARY_PAYABLE' AS NVARCHAR(50)), CAST(EMP_ID AS INT), CAST(FULL_NAME AS NVARCHAR(150)), 6
        FROM #SalarySplit WHERE NET_PAY < 0
        UNION ALL
