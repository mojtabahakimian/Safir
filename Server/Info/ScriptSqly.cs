...
BEGIN
    ...
    BEGIN
        ...
        -- فیکس: عدم دخالت حقوق رسمی در جمع مزایا (ناخالص پرداختی واقعی) تا فیش دوبرابر نشود
        SELECT @GROSS_PAY = ISNULL(SUM(AMOUNT), 0) FROM @ItemCalc WHERE ITEM_TYPE IN (1, 2) AND ITEM_CODE <> 'BASE_SAL_B';
