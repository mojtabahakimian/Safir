...
";
                ...
                // Snapshot columns are additive: finalized historical runs are never updated.
                db.Execute(@"IF COL_LENGTH('dbo.PAY2_RUN_DETAIL', 'NOMINAL_AMOUNT') IS NULL ALTER TABLE dbo.PAY2_RUN_DETAIL ADD NOMINAL_AMOUNT BIGINT NULL;
...
BEGIN
 ...
 VALUES ('SANOVAT_PAYE',N'پایه سنوات روزانه',1,1,1,1,1,2,1,1,3,1,N'مبلغ روزانه؛ مشمولیت از تعریف آیتم/حکم/Override قابل تغییر است');
END;");
...
BEGIN
    ...
    DECLARE @GROSS_PAY BIGINT, @NOMINAL_GROSS BIGINT, @INS_BASE BIGINT, @INS_WORKER BIGINT, @INS_EMPLOYER BIGINT;
    ...
    BEGIN
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        BEGIN
                            ...
                            SET @CALC_AMOUNT = CAST(ROUND((@DAILY_OFFICIAL * @PAY_DAYS * @ITEM_AMOUNT / 100.0), 0) AS BIGINT);
                            SET @INS_CALC_AMOUNT = CAST(ROUND((@DAILY_NOMINAL * @INS_DAYS * @ITEM_AMOUNT / 100.0), 0) AS BIGINT);
                        END
                    END
        ...
        BEGIN
            SET @FB_NOMINAL = ISNULL((SELECT TOP 1 CAST(DL.AMOUNT AS BIGINT) FROM PAY2_DECREE D INNER JOIN PAY2_DECREE_LINE DL ON D.DEC_ID = DL.DEC_ID INNER JOIN PAY2_ITEM_DEF ID ON DL.ITEM_ID = ID.ITEM_ID WHERE D.EMP_ID = @EMP_ID AND D.IS_CONFIRMED = 1 AND ID.ITEM_CODE = 'BASE_SAL_B' ORDER BY D.EFF_FROM DESC), 0);
            IF @FB_NOMINAL = 0
                SELECT TOP 1 @FB_NOMINAL = CAST(DL.AMOUNT AS BIGINT) FROM PAY2_DECREE D INNER JOIN PAY2_DECREE_LINE DL ON D.DEC_ID = DL.DEC_ID INNER JOIN PAY2_ITEM_DEF ID ON DL.ITEM_ID = ID.ITEM_ID WHERE D.EMP_ID = @EMP_ID AND D.IS_CONFIRMED = 1 AND ID.ITEM_CODE = 'BASE_SAL' ORDER BY D.EFF_FROM DESC;
        ...
        ELSE IF @OT_HOUR_BASE > 0
        BEGIN
            SET @FB_OFFICIAL = ISNULL((SELECT TOP 1 CAST(DL.AMOUNT AS BIGINT) FROM PAY2_DECREE D INNER JOIN PAY2_DECREE_LINE DL ON D.DEC_ID = DL.DEC_ID INNER JOIN PAY2_ITEM_DEF ID ON DL.ITEM_ID = ID.ITEM_ID WHERE D.EMP_ID = @EMP_ID AND D.IS_CONFIRMED = 1 AND ID.ITEM_CODE = 'BASE_SAL' ORDER BY D.EFF_FROM DESC), 0);
            IF @FB_OFFICIAL = 0
                SELECT TOP 1 @FB_OFFICIAL = CAST(DL.AMOUNT AS BIGINT) FROM PAY2_DECREE D INNER JOIN PAY2_DECREE_LINE DL ON D.DEC_ID = DL.DEC_ID INNER JOIN PAY2_ITEM_DEF ID ON DL.ITEM_ID = ID.ITEM_ID WHERE D.EMP_ID = @EMP_ID AND D.IS_CONFIRMED = 1 AND ID.ITEM_CODE = 'BASE_SAL_B' ORDER BY D.EFF_FROM DESC;
        ...
        INSERT INTO PAY2_RUN_LINE (
            ...
            NOMINAL_GROSS, NOMINAL_DAYS, ROUNDING_ADJ, HIRE_DATE_SNAP, FIRE_DATE_SNAP
        ) VALUES (
            ...
            @NOMINAL_GROSS, @DAYS, @ROUNDING_DIFF,
            (SELECT HIRE_DATE FROM PAY2_EMPLOYEE WHERE EMP_ID=@EMP_ID),
            (SELECT FIRE_DATE FROM PAY2_EMPLOYEE WHERE EMP_ID=@EMP_ID)
        );

        INSERT INTO PAY2_RUN_DETAIL (RUN_ID, EMP_ID, ITEM_ID, AMOUNT, NOMINAL_AMOUNT, ITEM_CODE_SNAP, CALC_BASIS_SNAP, ITEM_TYPE_SNAP, INS_SUBJECT, TAX_SUBJECT)
        SELECT @NEW_RUN_ID, @EMP_ID, C.ITEM_ID, SUM(C.AMOUNT), SUM(C.INS_AMOUNT), MAX(C.ITEM_CODE), MAX(I.CALC_BASIS), MAX(C.ITEM_TYPE), MAX(CAST(C.INS_SUBJECT AS INT)), MAX(CAST(C.TAX_SUBJECT AS INT))
        FROM @ItemCalc C INNER JOIN PAY2_ITEM_DEF I ON I.ITEM_ID=C.ITEM_ID
        GROUP BY C.ITEM_ID HAVING SUM(C.AMOUNT) <> 0 OR SUM(C.INS_AMOUNT) <> 0;
