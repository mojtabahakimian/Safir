...
CREATE OR ALTER PROCEDURE dbo.SP_PAY2_PREVIEW_INS_NON_SUBJECT_OPTIN
AS
BEGIN
 SET NOCOUNT ON;
 ...
 FROM dbo.PAY2_ITEM_TMPL_LINE TL JOIN dbo.PAY2_ITEM_DEF I ON I.ITEM_ID=TL.ITEM_ID
 WHERE I.ITEM_CODE IN('SHIFT','OT_NORMAL','OT_HOLIDAY','OT_ADMIN') AND TL.INS_OV IS NOT NULL;
END;
GO

CREATE OR ALTER PROCEDURE dbo.SP_PAY2_APPLY_INS_NON_SUBJECT_OPTIN @CONFIRM_DATABASE SYSNAME,@APPLIED_BY INT
AS
BEGIN
 SET NOCOUNT ON; SET XACT_ABORT ON;
 ...
 IF NOT EXISTS(SELECT 1 FROM dbo.PAY2_CONFIG WHERE CFG_KEY=N'INS_NON_SUBJECT_OPT_IN' AND CFG_VALUE=N'APPROVED:'+DB_NAME())
  THROW 51102,N'Opt-in این دیتابیس فعال نیست؛ ابتدا Preview و سپس مقدار APPROVED:<DatabaseName> را با تأیید صریح ثبت کنید.',1;
 IF EXISTS
 (
  SELECT 1 FROM dbo.PAY2_DECREE_LINE DL JOIN dbo.PAY2_ITEM_DEF I ON I.ITEM_ID=DL.ITEM_ID WHERE I.ITEM_CODE IN('SHIFT','OT_NORMAL','OT_HOLIDAY','OT_ADMIN') AND DL.INS_OV=1
  UNION ALL SELECT 1 FROM dbo.PAY2_OVERRIDE O JOIN dbo.PAY2_ITEM_DEF I ON I.ITEM_ID=O.ITEM_ID WHERE I.ITEM_CODE IN('SHIFT','OT_NORMAL','OT_HOLIDAY','OT_ADMIN') AND O.INS_OV=1
  UNION ALL SELECT 1 FROM dbo.PAY2_ITEM_TMPL_LINE TL JOIN dbo.PAY2_ITEM_DEF I ON I.ITEM_ID=TL.ITEM_ID WHERE I.ITEM_CODE IN('SHIFT','OT_NORMAL','OT_HOLIDAY','OT_ADMIN') AND TL.INS_OV=1
 ) THROW 51103,N'Override مشمول بیمه وجود دارد؛ Preview را بررسی و Overrideها را صریحاً تعیین تکلیف کنید.',1;
