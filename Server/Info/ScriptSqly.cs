...
BEGIN
    ...
    SELECT
        ...
        CAST(CASE WHEN B.WORK_DAYS = 0 AND B.NET_PAY < 0
                       AND B.LOAN_DED = 0 AND B.ADVANCE_DED = 0 AND B.OTHER_DED = 0
                  THEN 1 ELSE 0 END AS BIT),
        CASE WHEN B.WORK_DAYS = 0 AND B.NET_PAY < 0
                  AND B.LOAN_DED = 0 AND B.ADVANCE_DED = 0 AND B.OTHER_DED = 0
             THEN 0 WHEN B.DAYS_TOLID > 0 THEN B.R_T + (B.GROSS_PAY - (B.R_T + B.R_E + B.R_F + B.R_K)) ELSE B.R_T END,
        CASE WHEN B.WORK_DAYS = 0 AND B.NET_PAY < 0 AND B.LOAN_DED = 0 AND B.ADVANCE_DED = 0 AND B.OTHER_DED = 0 THEN 0 WHEN B.DAYS_TOLID = 0 AND B.DAYS_EDARI > 0 THEN B.R_E + (B.GROSS_PAY - (B.R_T + B.R_E + B.R_F + B.R_K)) ELSE B.R_E END,
        CASE WHEN B.WORK_DAYS = 0 AND B.NET_PAY < 0 AND B.LOAN_DED = 0 AND B.ADVANCE_DED = 0 AND B.OTHER_DED = 0 THEN 0 WHEN B.DAYS_TOLID = 0 AND B.DAYS_EDARI = 0 AND B.DAYS_FOROSH > 0 THEN B.R_F + (B.GROSS_PAY - (B.R_T + B.R_E + B.R_F + B.R_K)) ELSE B.R_F END,
        CASE WHEN B.WORK_DAYS = 0 AND B.NET_PAY < 0 AND B.LOAN_DED = 0 AND B.ADVANCE_DED = 0 AND B.OTHER_DED = 0 THEN 0 WHEN B.DAYS_TOLID = 0 AND B.DAYS_EDARI = 0 AND B.DAYS_FOROSH = 0 THEN B.R_K + (B.GROSS_PAY - (B.R_T + B.R_E + B.R_F + B.R_K)) ELSE B.R_K END,
