...
";
        ...
        /// <summary>
        /// نصب ماژول «بستن ماه بهای تمام‌شده» (CostClose) — ۱۶ جدول CC_*، رویه‌ها
        /// و داده اولیه. معادل دقیق فایل‌های Server/Database/costclose_*.sql،
        /// به همان ترتیب اجرا می‌شود. تمام رویه‌ها CREATE OR ALTER یا IF OBJECT_ID
        /// IS NULL هستند، پس اجرای مکرر بی‌خطر است.
        /// </summary>
        private static void CostCloseScript(bool isCustomCall, SqlConnection db)
        {
            if (!isCustomCall) return;

            // ───── costclose_schema.sql ─────
            string costCloseSchema = @"
-- ترتیب اجرای فایل‌های ماژول CostClose (باید دقیقاً به همین ترتیب اجرا شوند):
--    1. costclose_schema.sql
--    2. costclose_seed_data.sql
--    3. costclose_procedures_phase1.sql
--    4. costclose_chk04_autofix.sql
--    5. costclose_s05_gate.sql
--    6. costclose_rate_engine.sql
--    7. costclose_rollback.sql
--    8. costclose_variance_steps.sql
--    9. costclose_margin_report_approve.sql
--   10. costclose_margin_fix_kalas.sql
--   نکته: فایل ۱۰ باید پس از ۹ (costclose_margin_report_approve.sql) اجرا شود، چون رویه S12 را جایگزین می‌کند.
...
GO
";
            ExecuteBatches(db, costCloseSchema);

            // ───── costclose_seed_data.sql ─────
            string costCloseSeedData = @"
/* ═══════════════════════════════════════════════════════════════════
...
GO
";
            ExecuteBatches(db, costCloseSeedData);

            // ───── costclose_procedures_phase1.sql ─────
            string costCloseProceduresPhase1 = @"
/* ═══════════════════════════════════════════════════════════════════
...
-- پاکسازی یک رویه آزمایشی قدیمی که دیگر استفاده نمی‌شود. اگر هرگز
-- ساخته نشده باشد هم خطا نمی‌دهد.
...
BEGIN
    ...
    SELECT @total AS کل_اسناد, @changed AS تعداد_بازشماره_شده;
END
GO
...
GO
";
            ExecuteBatches(db, costCloseProceduresPhase1);

            // ───── costclose_chk04_autofix.sql ─────
            string costCloseChk04Autofix = @"
/* ═══════════════════════════════════════════════════════════════════
...
GO
";
            ExecuteBatches(db, costCloseChk04Autofix);

            // ───── costclose_s05_gate.sql ─────
            string costCloseS05Gate = @"
/* ═══════════════════════════════════════════════════════════════════
...
GO
";
            ExecuteBatches(db, costCloseS05Gate);

            // ───── costclose_rate_engine.sql ─────
            string costCloseRateEngine = @"
/* ═══════════════════════════════════════════════════════════════════
...
GO
";
            ExecuteBatches(db, costCloseRateEngine);

            // ───── costclose_rollback.sql ─────
            string costCloseRollback = @"
/* ═══════════════════════════════════════════════════════════════════
...
GO
";
            ExecuteBatches(db, costCloseRollback);

            // ───── costclose_variance_steps.sql ─────
            string costCloseVarianceSteps = @"
/* ═══════════════════════════════════════════════════════════════════
...
BEGIN
    ...
    SELECT @deleted AS حذف_شده, @inserted AS درج_شده, @countRows AS انبارگرداني;
END
GO
...
BEGIN
    ...
    SELECT  COUNT(*)                                        AS تعداد_کالا,
            SUM(CASE WHEN IsKeyItem = 1 THEN 1 ELSE 0 END)  AS کالاي_کليدي,
            SUM(ISNULL(AmountVariance, 0))                  AS جمع_خالص,
            SUM(ABS(ISNULL(AmountVariance, 0)))             AS جمع_قدرمطلق
    FROM    dbo.CC_Variance WHERE RunId = @RunId;
END
GO
...
BEGIN
    ...
    SELECT @n AS تعداد_سطر_به‌روز_شده;
END
GO
...
GO
";
            ExecuteBatches(db, costCloseVarianceSteps);

            // ───── costclose_margin_report_approve.sql ─────
            string costCloseMarginReportApprove = @"
/* ═══════════════════════════════════════════════════════════════════
...
GO
";
            ExecuteBatches(db, costCloseMarginReportApprove);

            // ───── costclose_margin_fix_kalas.sql ─────
            string costCloseMarginFixKalas = @"
/* ═══════════════════════════════════════════════════════════════════
...
GO
";
            ExecuteBatches(db, costCloseMarginFixKalas);

            db.Execute("PRINT N'ماژول بستن ماه بهای تمام‌شده نصب/به‌روزرسانی شد.';");
        }
