...
BEGIN
    ...
    BEGIN
        ...
        -- A zero-work insurance-only person owes the exact insurance/tax amount.
        -- Rounding that debt used to manufacture a small positive GROSS_PAY (for
        -- example 257 rials) which was then posted as a fictitious salary expense.
        IF @ROUND_MODE > 1
           AND NOT (@DAYSB = 0 AND @GROSS_PAY = 0
                    AND @LOAN_DED = 0 AND @ADVANCE_DED = 0 AND @OTHER_DED = 0)
...
BEGIN
    ...
    IF @ACC_INS_EXP IS NULL AND EXISTS (
        SELECT 1 FROM PAY2_RUN_LINE
        WHERE RUN_ID = @RUN_ID AND INS_EMPLOYER > 0
          AND NOT (WORK_DAYS = 0 AND GROSS_PAY = 0 AND NET_PAY < 0
                   AND LOAN_DED = 0 AND ADVANCE_DED = 0 AND OTHER_DED = 0)
    ) SET @MissingAcc += N'هزینه بیمه کارفرما، ';
    ...
    SplitBase AS (
        SELECT
            RL.EMP_ID, RL.WORK_DAYS, RL.GROSS_PAY, A.DAYS_TOLID, A.DAYS_EDARI, A.DAYS_FOROSH, A.DAYS_KHADAMAT,
    ...
    INSERT INTO #SalarySplit (
        EMP_ID, FULL_NAME, ACC_T, IS_INSURANCE_ONLY, EXP_TOLID, EXP_EDARI, EXP_FOROSH, EXP_KHADAMAT,
        NET_PAY, INS_WORKER, INS_EMPLOYER, TAX_AMOUNT, LOAN_DED, ADVANCE_DED, OTHER_DED
    )
    SELECT
        B.EMP_ID, E.FULL_NAME, E.ACC_T,
        CAST(CASE WHEN B.WORK_DAYS = 0 AND B.GROSS_PAY = 0 AND B.NET_PAY < 0
                       AND B.LOAN_DED = 0 AND B.ADVANCE_DED = 0 AND B.OTHER_DED = 0
                  THEN 1 ELSE 0 END AS BIT),
        CASE WHEN B.WORK_DAYS = 0 AND B.GROSS_PAY = 0 AND B.NET_PAY < 0
                  AND B.LOAN_DED = 0 AND B.ADVANCE_DED = 0 AND B.OTHER_DED = 0
             THEN 0 WHEN B.DAYS_TOLID > 0 THEN B.R_T + (B.GROSS_PAY - (B.R_T + B.R_E + B.R_F + B.R_K)) ELSE B.R_T END,
        CASE WHEN B.WORK_DAYS = 0 AND B.GROSS_PAY = 0 AND B.NET_PAY < 0 AND B.LOAN_DED = 0 AND B.ADVANCE_DED = 0 AND B.OTHER_DED = 0 THEN 0 WHEN B.DAYS_TOLID = 0 AND B.DAYS_EDARI > 0 THEN B.R_E + (B.GROSS_PAY - (B.R_T + B.R_E + B.R_F + B.R_K)) ELSE B.R_E END,
        CASE WHEN B.WORK_DAYS = 0 AND B.GROSS_PAY = 0 AND B.NET_PAY < 0 AND B.LOAN_DED = 0 AND B.ADVANCE_DED = 0 AND B.OTHER_DED = 0 THEN 0 WHEN B.DAYS_TOLID = 0 AND B.DAYS_EDARI = 0 AND B.DAYS_FOROSH > 0 THEN B.R_F + (B.GROSS_PAY - (B.R_T + B.R_E + B.R_F + B.R_K)) ELSE B.R_F END,
        CASE WHEN B.WORK_DAYS = 0 AND B.GROSS_PAY = 0 AND B.NET_PAY < 0 AND B.LOAN_DED = 0 AND B.ADVANCE_DED = 0 AND B.OTHER_DED = 0 THEN 0 WHEN B.DAYS_TOLID = 0 AND B.DAYS_EDARI = 0 AND B.DAYS_FOROSH = 0 THEN B.R_K + (B.GROSS_PAY - (B.R_T + B.R_E + B.R_F + B.R_K)) ELSE B.R_K END,
    ...
    BEGIN
        ...
        UNION ALL
        SELECT CAST(@ACC_INS_EXP AS NVARCHAR(100)), CAST(N'هزینه بیمه کارفرما ' + @ML AS NVARCHAR(500)), CAST(SUM(CASE WHEN IS_INSURANCE_ONLY = 0 THEN INS_EMPLOYER ELSE 0 END) AS BIGINT), CAST(0 AS BIGINT), CAST('INS_EXP' AS NVARCHAR(50)), CAST(NULL AS INT), CAST(NULL AS NVARCHAR(150)), 5
        FROM #SalarySplit HAVING SUM(CASE WHEN IS_INSURANCE_ONLY = 0 THEN INS_EMPLOYER ELSE 0 END) > 0
        UNION ALL
        ...
        UNION ALL
        SELECT CAST(@ACC_SALARY_PAY AS NVARCHAR(100)), CAST(N'بدهی بیمه و مالیات پرسنل ' + @ML AS NVARCHAR(500)), CAST(SUM(CASE WHEN IS_INSURANCE_ONLY = 1 THEN INS_WORKER + INS_EMPLOYER + TAX_AMOUNT WHEN NET_PAY < 0 THEN -NET_PAY ELSE 0 END) AS BIGINT), CAST(0 AS BIGINT), CAST('SALARY_PAYABLE' AS NVARCHAR(50)), CAST(NULL AS INT), CAST(NULL AS NVARCHAR(150)), 6
        FROM #SalarySplit HAVING SUM(CASE WHEN IS_INSURANCE_ONLY = 1 THEN INS_WORKER + INS_EMPLOYER + TAX_AMOUNT WHEN NET_PAY < 0 THEN -NET_PAY ELSE 0 END) > 0
        UNION ALL
    ...
    BEGIN
        ...
        UNION ALL
        SELECT CAST(ACC_T AS NVARCHAR(100)), CAST(N'بدهی بیمه و مالیات: ' + @ML + N' | ' + FULL_NAME AS NVARCHAR(500)), CAST(CASE WHEN IS_INSURANCE_ONLY = 1 THEN INS_WORKER + INS_EMPLOYER + TAX_AMOUNT ELSE -NET_PAY END AS BIGINT), CAST(0 AS BIGINT), CAST('SALARY_PAYABLE' AS NVARCHAR(50)), CAST(EMP_ID AS INT), CAST(FULL_NAME AS NVARCHAR(150)), 6
        FROM #SalarySplit WHERE NET_PAY < 0
        UNION ALL
