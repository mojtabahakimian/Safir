...
BEGIN
    ...
    DECLARE @HAS_BOTH_SAL BIT, @DAILY_NOMINAL DECIMAL(18,2), @DAILY_OFFICIAL DECIMAL(18,2), @DAILY_SEN_NOMINAL DECIMAL(18,2), @DAILY_SEN_OFFICIAL DECIMAL(18,2);
    ...
    BEGIN
        ...
        IF EXISTS (SELECT 1 FROM @ItemCalc WHERE ITEM_CODE='BASE_SAL' AND INS_AMOUNT<>0) AND EXISTS (SELECT 1 FROM @ItemCalc WHERE ITEM_CODE='BASE_SAL_B' AND AMOUNT<>0)
            SET @HAS_BOTH_SAL = 1;
        ...
        BEGIN
            ...
            RETURN;
        END;
        IF NOT EXISTS (SELECT 1 FROM @ItemCalc WHERE ITEM_CODE='BASE_SAL_B' AND AMOUNT<>0)
        BEGIN
