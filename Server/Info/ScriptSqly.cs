...
('SANOVAT_PAYE',N'پایه سنوات',                1, 1, 1, 1, 1, 2, 1, 25);   -- پایه سنوات روزانه؛ مشمولیت طبق تنظیمات آیتم
END;
GO
...
";
                ...
                -- پایه سنوات (SANOVAT_PAYE): افزودن Idempotent برای دیتابیس‌های موجود
                IF NOT EXISTS (SELECT 1 FROM PAY2_ITEM_DEF WHERE ITEM_CODE = 'SANOVAT_PAYE')
                    INSERT INTO PAY2_ITEM_DEF (ITEM_CODE, ITEM_NAME, ITEM_TYPE, CALC_BASIS, INS_SUBJECT, TAX_SUBJECT, INS_BASE_DAYS, PAY_BASE_DAYS, IS_SYSTEM, SORT_ORDER)
                    VALUES ('SANOVAT_PAYE', N'پایه سنوات', 1, 1, 1, 1, 1, 2, 1, 25);
                ";
...
BEGIN
    ...
    BEGIN
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        ELSE
                        BEGIN
                            -- حق شیفت پرداختی از حقوق رسمی (BASE_SAL_B)، مبنای بیمه از حقوق اسمی (BASE_SAL)
        ...
        -- ریل پرداخت اضافه‌کار بر اساس حقوق رسمی BASE_SAL_B (فیش/پرداخت واقعی)
        ...
        -- ریل بیمه و مالیات اضافه‌کار بر اساس حقوق اسمی BASE_SAL
        ...
        -- ناخالص پرداختی بر اساس حقوق رسمی BASE_SAL_B (با جلوگیری از دوبارشماری؛ BASE_SAL و BASE_SAL_B هرگز با هم جمع نمی‌شوند)
        SELECT @GROSS_PAY = ISNULL(SUM(AMOUNT), 0) 
        FROM @ItemCalc 
        WHERE ITEM_TYPE IN (1, 2) AND (@HAS_BOTH_SAL = 0 OR ITEM_CODE <> 'BASE_SAL');

        -- 🚀 مبنای بیمه بر اساس حقوق اسمی BASE_SAL (با استفاده از INS_AMOUNT)
        SET @INS_OFFICIAL_VALID = 0; SET @INS_DROP_SAL = NULL;
        IF @HAS_BOTH_SAL = 1
        BEGIN
            IF EXISTS (SELECT 1 FROM @ItemCalc WHERE ITEM_CODE = 'BASE_SAL' AND INS_SUBJECT = 1 AND INS_AMOUNT <> 0)
                SET @INS_OFFICIAL_VALID = 1;
            SET @INS_DROP_SAL = CASE WHEN @INS_OFFICIAL_VALID = 1 THEN 'BASE_SAL_B' ELSE 'BASE_SAL' END;
        END;
        ...
        ELSE
        BEGIN
            -- 🚀 مالیات کاملاً بر اساس حقوق اسمی BASE_SAL و مقادیر INS_AMOUNT محاسبه می‌شود
            SET @TAX_OFFICIAL_VALID = 0; SET @TAX_DROP_SAL = NULL;
            IF @HAS_BOTH_SAL = 1
            BEGIN
                IF EXISTS (SELECT 1 FROM @ItemCalc WHERE ITEM_CODE = 'BASE_SAL' AND TAX_SUBJECT = 1 AND INS_AMOUNT <> 0)
                    SET @TAX_OFFICIAL_VALID = 1;
                SET @TAX_DROP_SAL = CASE WHEN @TAX_OFFICIAL_VALID = 1 THEN 'BASE_SAL_B' ELSE 'BASE_SAL' END;
            END;
...
BEGIN
   ...
   -- سابقه کل بر اساس استاندارد ۳۶۵ روزه (جایگزین خط اشتباه قبلی) 
    ...
    -- تسویه = پرداخت واقعی → ریل رسمی (BASE_SAL_B). اگر هر دو ریل باشند، ریل اسمی (BASE_SAL)
    -- کنار می‌رود تا این دو مبلغ هرگز با هم جمع نشوند؛ اگر فقط اسمی باشد، خودش fallback می‌شود.
    DECLARE @SET_HAS_BOTH_SAL BIT =
        CASE WHEN EXISTS (SELECT 1 FROM PAY2_DECREE_LINE DL INNER JOIN PAY2_ITEM_DEF ID ON DL.ITEM_ID = ID.ITEM_ID WHERE DL.DEC_ID = @LAST_DEC_ID AND ID.ITEM_CODE = 'BASE_SAL')
              AND  EXISTS (SELECT 1 FROM PAY2_DECREE_LINE DL INNER JOIN PAY2_ITEM_DEF ID ON DL.ITEM_ID = ID.ITEM_ID WHERE DL.DEC_ID = @LAST_DEC_ID AND ID.ITEM_CODE = 'BASE_SAL_B')
             THEN 1 ELSE 0 END;

    DECLARE @LAST_DAILY_ONLY BIGINT = ISNULL((SELECT SUM(DL.AMOUNT) FROM PAY2_DECREE_LINE DL INNER JOIN PAY2_ITEM_DEF ID ON DL.ITEM_ID = ID.ITEM_ID WHERE DL.DEC_ID = @LAST_DEC_ID AND ID.ITEM_TYPE = 1 AND ID.INS_SUBJECT = 1 AND ID.CALC_BASIS = 1 AND (@SET_HAS_BOTH_SAL = 0 OR ID.ITEM_CODE <> 'BASE_SAL')), 0);
    DECLARE @LAST_MONTHLY_ONLY BIGINT = ISNULL((SELECT SUM(DL.AMOUNT) FROM PAY2_DECREE_LINE DL INNER JOIN PAY2_ITEM_DEF ID ON DL.ITEM_ID = ID.ITEM_ID WHERE DL.DEC_ID = @LAST_DEC_ID AND ID.ITEM_TYPE = 1 AND ID.INS_SUBJECT = 1 AND ID.CALC_BASIS = 2 AND (@SET_HAS_BOTH_SAL = 0 OR ID.ITEM_CODE <> 'BASE_SAL')), 0);
