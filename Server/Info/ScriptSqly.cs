...
BEGIN
    ...
    BEGIN
        ...
        FROM @ItemCalc 
        WHERE INS_SUBJECT = 1 AND ITEM_TYPE IN (1, 2) AND (@HAS_BOTH_SAL = 0 OR ITEM_CODE <> 'BASE_SAL');
        ...
        BEGIN
            ...
            FROM @ItemCalc 
            WHERE TAX_SUBJECT = 1 AND ITEM_TYPE IN (1, 2) AND (@HAS_BOTH_SAL = 0 OR ITEM_CODE <> 'BASE_SAL');
...
BEGIN
    ...
    UNION ALL 
    SELECT @ACC_SALARY_PAY, N'پرداختنی حقوق خالص: ' + E.LAST_NAME + N' ' + E.FIRST_NAME, 0, RL.NET_PAY, 'SALARY_PAYABLE', E.EMP_ID
    ...
    UNION ALL 
    SELECT @ACC_LOAN_HES, N'کسر اقساط وام: ' + E.LAST_NAME + N' ' + E.FIRST_NAME, 0, RL.LOAN_DED, 'LOAN_HES', E.EMP_ID
    FROM PAY2_RUN_LINE RL INNER JOIN PAY2_EMPLOYEE E ON RL.EMP_ID = E.EMP_ID
...
BEGIN
    ...
    UNION ALL
    SELECT @ACC_LOAN_HES, N'وصول مانده وام از تسویه: ' + @EMP_NAME, 0, LOAN_BALANCE, 'LOAN_COLLECT', @EMP_ID 
    FROM PAY2_SETTLEMENT WHERE SET_ID=@SET_ID AND LOAN_BALANCE > 0
    UNION ALL
    SELECT @ACC_ADV_HES, N'وصول بدهکاری (مساعده): ' + @EMP_NAME, 0, PREV_DEBIT, 'ADV_COLLECT', @EMP_ID 
    FROM PAY2_SETTLEMENT WHERE SET_ID=@SET_ID AND PREV_DEBIT > 0
    UNION ALL
