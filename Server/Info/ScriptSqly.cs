...
BEGIN
    ...
    BEGIN
        ...
        SELECT @NEW_RUN_ID, @EMP_ID, C.ITEM_ID, SUM(C.AMOUNT), SUM(C.INS_AMOUNT), MAX(C.ITEM_CODE), MAX(I.CALC_BASIS), MAX(C.ITEM_TYPE),
               SUM(CASE WHEN C.INS_SUBJECT=1 THEN C.INS_AMOUNT ELSE 0 END), SUM(CASE WHEN C.TAX_SUBJECT=1 THEN C.INS_AMOUNT ELSE 0 END),
               CASE WHEN SUM(CASE WHEN C.INS_SUBJECT=1 THEN C.INS_AMOUNT ELSE 0 END)<>0 THEN 1 ELSE 0 END,
               CASE WHEN SUM(CASE WHEN C.TAX_SUBJECT=1 THEN C.INS_AMOUNT ELSE 0 END)<>0 THEN 1 ELSE 0 END
