...
BEGIN
    ...
    BEGIN
                ...
                BEGIN
                    ...
                    BEGIN
                        ...
                        ELSE
                            ...
                            SET @CALC_AMOUNT = CAST(ROUND((@CURRENT_DEC_DAILY_BASE * @ITEM_AMOUNT / 100.0) * @PAY_DAYS, 0) AS BIGINT);
                    END
                    ...
                    -- اصلاح باگ ۳۰برابری: آیتم‌های با مبنای روزانه مستقیماً در روزهای کارکرد ضرب می‌شوند
        ...
        BEGIN
            -- Fallback: مبلغ پایه خام روزانه از آخرین حکم
            DECLARE @FALLBACK_DAILY_BASE BIGINT = 0;
            SELECT TOP 1 @FALLBACK_DAILY_BASE = CAST(DL.AMOUNT AS BIGINT)
            ...
            SET @EFFECTIVE_HOURLY = ISNULL(CAST(@FALLBACK_DAILY_BASE AS DECIMAL(18,2)) / NULLIF(@OT_HOUR_BASE, 0), 0);
        END
...
BEGIN
    ...
    DECLARE @LAST_DAILY_BASE BIGINT = ISNULL((SELECT SUM(DL.AMOUNT) FROM PAY2_DECREE_LINE DL INNER JOIN PAY2_ITEM_DEF ID ON DL.ITEM_ID = ID.ITEM_ID WHERE DL.DEC_ID = @LAST_DEC_ID AND ID.ITEM_TYPE = 1 AND ID.INS_SUBJECT = 1), 0);
    DECLARE @LAST_SALARY BIGINT = @LAST_DAILY_BASE * 30;
    DECLARE @LAST_DAILY BIGINT = @LAST_DAILY_BASE;
