...
namespace Safir.Client.Services
{
    public class Pay2RunApiService
    {
        ...
        public async Task<byte[]> GetSmartExcelAsync(int runId)
        {
            var res = await _http.GetAsync($"api/pay2/run/{runId}/smart-excel");
            if (!res.IsSuccessStatusCode)
            {
                var err = await res.Content.ReadAsStringAsync();
                throw new Exception(string.IsNullOrWhiteSpace(err)
                    ? $"خطا در دریافت اکسل هوشمند (کد {(int)res.StatusCode})."
                    : err);
            }
